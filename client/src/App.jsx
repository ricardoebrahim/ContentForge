...
function App() {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://contentforge-production-cd34.up.railway.app${endpoint}`, {
        method: 'POST',
  ...
  const deleteHistory = async (id) => {
    try {
      await fetch(`https://contentforge-production-cd34.up.railway.app/api/history/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      })
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setResult(parsed)
        await fetch('https://contentforge-production-cd34.up.railway.app/api/history', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
            ...
            {history.length === 0 ? (
              ...
              <div className="space-y-2">
                {history.map((item) => (
                  <div
  key={item._id}
  onClick={() => {setResult({ summary: item.summary, social: item.social, seo: item.seo }); setCopied('')}}
